/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AppointmentEmployeeWhereInput } from "./AppointmentEmployeeWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { AppointmentEmployeeOrderByInput } from "./AppointmentEmployeeOrderByInput";

@ArgsType()
class AppointmentEmployeeFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => AppointmentEmployeeWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => AppointmentEmployeeWhereInput, { nullable: true })
  @Type(() => AppointmentEmployeeWhereInput)
  where?: AppointmentEmployeeWhereInput;

  @ApiProperty({
    required: false,
    type: [AppointmentEmployeeOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [AppointmentEmployeeOrderByInput], { nullable: true })
  @Type(() => AppointmentEmployeeOrderByInput)
  orderBy?: Array<AppointmentEmployeeOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { AppointmentEmployeeFindManyArgs as AppointmentEmployeeFindManyArgs };
