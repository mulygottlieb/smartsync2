/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CandidatePersonalQualityWhereUniqueInput } from "./CandidatePersonalQualityWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CandidatePersonalQualityUpdateInput } from "./CandidatePersonalQualityUpdateInput";

@ArgsType()
class UpdateCandidatePersonalQualityArgs {
  @ApiProperty({
    required: true,
    type: () => CandidatePersonalQualityWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CandidatePersonalQualityWhereUniqueInput)
  @Field(() => CandidatePersonalQualityWhereUniqueInput, { nullable: false })
  where!: CandidatePersonalQualityWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CandidatePersonalQualityUpdateInput,
  })
  @ValidateNested()
  @Type(() => CandidatePersonalQualityUpdateInput)
  @Field(() => CandidatePersonalQualityUpdateInput, { nullable: false })
  data!: CandidatePersonalQualityUpdateInput;
}

export { UpdateCandidatePersonalQualityArgs as UpdateCandidatePersonalQualityArgs };
