/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CandidateTechnologyWhereInput } from "./CandidateTechnologyWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class CandidateTechnologyListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => CandidateTechnologyWhereInput,
  })
  @ValidateNested()
  @Type(() => CandidateTechnologyWhereInput)
  @IsOptional()
  @Field(() => CandidateTechnologyWhereInput, {
    nullable: true,
  })
  every?: CandidateTechnologyWhereInput;

  @ApiProperty({
    required: false,
    type: () => CandidateTechnologyWhereInput,
  })
  @ValidateNested()
  @Type(() => CandidateTechnologyWhereInput)
  @IsOptional()
  @Field(() => CandidateTechnologyWhereInput, {
    nullable: true,
  })
  some?: CandidateTechnologyWhereInput;

  @ApiProperty({
    required: false,
    type: () => CandidateTechnologyWhereInput,
  })
  @ValidateNested()
  @Type(() => CandidateTechnologyWhereInput)
  @IsOptional()
  @Field(() => CandidateTechnologyWhereInput, {
    nullable: true,
  })
  none?: CandidateTechnologyWhereInput;
}
export { CandidateTechnologyListRelationFilter as CandidateTechnologyListRelationFilter };
