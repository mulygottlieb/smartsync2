/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EquipmentSharingPolicyWhereUniqueInput } from "./EquipmentSharingPolicyWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { EquipmentSharingPolicyUpdateInput } from "./EquipmentSharingPolicyUpdateInput";

@ArgsType()
class UpdateEquipmentSharingPolicyArgs {
  @ApiProperty({
    required: true,
    type: () => EquipmentSharingPolicyWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => EquipmentSharingPolicyWhereUniqueInput)
  @Field(() => EquipmentSharingPolicyWhereUniqueInput, { nullable: false })
  where!: EquipmentSharingPolicyWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => EquipmentSharingPolicyUpdateInput,
  })
  @ValidateNested()
  @Type(() => EquipmentSharingPolicyUpdateInput)
  @Field(() => EquipmentSharingPolicyUpdateInput, { nullable: false })
  data!: EquipmentSharingPolicyUpdateInput;
}

export { UpdateEquipmentSharingPolicyArgs as UpdateEquipmentSharingPolicyArgs };
