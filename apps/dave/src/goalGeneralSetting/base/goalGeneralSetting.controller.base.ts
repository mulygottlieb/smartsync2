/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import { GoalGeneralSettingService } from "../goalGeneralSetting.service";
import { GoalGeneralSettingCreateInput } from "./GoalGeneralSettingCreateInput";
import { GoalGeneralSettingWhereInput } from "./GoalGeneralSettingWhereInput";
import { GoalGeneralSettingWhereUniqueInput } from "./GoalGeneralSettingWhereUniqueInput";
import { GoalGeneralSettingFindManyArgs } from "./GoalGeneralSettingFindManyArgs";
import { GoalGeneralSettingUpdateInput } from "./GoalGeneralSettingUpdateInput";
import { GoalGeneralSetting } from "./GoalGeneralSetting";

export class GoalGeneralSettingControllerBase {
  constructor(protected readonly service: GoalGeneralSettingService) {}
  @common.Post()
  @swagger.ApiCreatedResponse({ type: GoalGeneralSetting })
  async create(
    @common.Body() data: GoalGeneralSettingCreateInput
  ): Promise<GoalGeneralSetting> {
    return await this.service.create({
      data: {
        ...data,

        tenant: data.tenant
          ? {
              connect: data.tenant,
            }
          : undefined,

        organization: data.organization
          ? {
              connect: data.organization,
            }
          : undefined,
      },
      select: {
        id: true,
        createdAt: true,
        updatedAt: true,
        maxObjectives: true,
        maxKeyResults: true,
        employeeCanCreateObjective: true,
        canOwnObjectives: true,
        canOwnKeyResult: true,
        krTypeKPI: true,
        krTypeTask: true,

        tenant: {
          select: {
            id: true,
          },
        },

        organization: {
          select: {
            id: true,
          },
        },
      },
    });
  }

  @common.Get()
  @swagger.ApiOkResponse({ type: [GoalGeneralSetting] })
  @ApiNestedQuery(GoalGeneralSettingFindManyArgs)
  async findMany(
    @common.Req() request: Request
  ): Promise<GoalGeneralSetting[]> {
    const args = plainToClass(GoalGeneralSettingFindManyArgs, request.query);
    return this.service.findMany({
      ...args,
      select: {
        id: true,
        createdAt: true,
        updatedAt: true,
        maxObjectives: true,
        maxKeyResults: true,
        employeeCanCreateObjective: true,
        canOwnObjectives: true,
        canOwnKeyResult: true,
        krTypeKPI: true,
        krTypeTask: true,

        tenant: {
          select: {
            id: true,
          },
        },

        organization: {
          select: {
            id: true,
          },
        },
      },
    });
  }

  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: GoalGeneralSetting })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async findOne(
    @common.Param() params: GoalGeneralSettingWhereUniqueInput
  ): Promise<GoalGeneralSetting | null> {
    const result = await this.service.findOne({
      where: params,
      select: {
        id: true,
        createdAt: true,
        updatedAt: true,
        maxObjectives: true,
        maxKeyResults: true,
        employeeCanCreateObjective: true,
        canOwnObjectives: true,
        canOwnKeyResult: true,
        krTypeKPI: true,
        krTypeTask: true,

        tenant: {
          select: {
            id: true,
          },
        },

        organization: {
          select: {
            id: true,
          },
        },
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: GoalGeneralSetting })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async update(
    @common.Param() params: GoalGeneralSettingWhereUniqueInput,
    @common.Body() data: GoalGeneralSettingUpdateInput
  ): Promise<GoalGeneralSetting | null> {
    try {
      return await this.service.update({
        where: params,
        data: {
          ...data,

          tenant: data.tenant
            ? {
                connect: data.tenant,
              }
            : undefined,

          organization: data.organization
            ? {
                connect: data.organization,
              }
            : undefined,
        },
        select: {
          id: true,
          createdAt: true,
          updatedAt: true,
          maxObjectives: true,
          maxKeyResults: true,
          employeeCanCreateObjective: true,
          canOwnObjectives: true,
          canOwnKeyResult: true,
          krTypeKPI: true,
          krTypeTask: true,

          tenant: {
            select: {
              id: true,
            },
          },

          organization: {
            select: {
              id: true,
            },
          },
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: GoalGeneralSetting })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async delete(
    @common.Param() params: GoalGeneralSettingWhereUniqueInput
  ): Promise<GoalGeneralSetting | null> {
    try {
      return await this.service.delete({
        where: params,
        select: {
          id: true,
          createdAt: true,
          updatedAt: true,
          maxObjectives: true,
          maxKeyResults: true,
          employeeCanCreateObjective: true,
          canOwnObjectives: true,
          canOwnKeyResult: true,
          krTypeKPI: true,
          krTypeTask: true,

          tenant: {
            select: {
              id: true,
            },
          },

          organization: {
            select: {
              id: true,
            },
          },
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
