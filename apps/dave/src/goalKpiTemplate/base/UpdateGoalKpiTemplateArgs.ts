/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { GoalKpiTemplateWhereUniqueInput } from "./GoalKpiTemplateWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { GoalKpiTemplateUpdateInput } from "./GoalKpiTemplateUpdateInput";

@ArgsType()
class UpdateGoalKpiTemplateArgs {
  @ApiProperty({
    required: true,
    type: () => GoalKpiTemplateWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => GoalKpiTemplateWhereUniqueInput)
  @Field(() => GoalKpiTemplateWhereUniqueInput, { nullable: false })
  where!: GoalKpiTemplateWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => GoalKpiTemplateUpdateInput,
  })
  @ValidateNested()
  @Type(() => GoalKpiTemplateUpdateInput)
  @Field(() => GoalKpiTemplateUpdateInput, { nullable: false })
  data!: GoalKpiTemplateUpdateInput;
}

export { UpdateGoalKpiTemplateArgs as UpdateGoalKpiTemplateArgs };
