/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { JobSearchCategoryWhereInput } from "./JobSearchCategoryWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { JobSearchCategoryOrderByInput } from "./JobSearchCategoryOrderByInput";

@ArgsType()
class JobSearchCategoryFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => JobSearchCategoryWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => JobSearchCategoryWhereInput, { nullable: true })
  @Type(() => JobSearchCategoryWhereInput)
  where?: JobSearchCategoryWhereInput;

  @ApiProperty({
    required: false,
    type: [JobSearchCategoryOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [JobSearchCategoryOrderByInput], { nullable: true })
  @Type(() => JobSearchCategoryOrderByInput)
  orderBy?: Array<JobSearchCategoryOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { JobSearchCategoryFindManyArgs as JobSearchCategoryFindManyArgs };
