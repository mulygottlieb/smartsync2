/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { EquipmentSharingPolicyWhereUniqueInput } from "../../equipmentSharingPolicy/base/EquipmentSharingPolicyWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class EquipmentSharingPolicyUpdateManyWithoutOrganizationsInput {
  @Field(() => [EquipmentSharingPolicyWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [EquipmentSharingPolicyWhereUniqueInput],
  })
  connect?: Array<EquipmentSharingPolicyWhereUniqueInput>;

  @Field(() => [EquipmentSharingPolicyWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [EquipmentSharingPolicyWhereUniqueInput],
  })
  disconnect?: Array<EquipmentSharingPolicyWhereUniqueInput>;

  @Field(() => [EquipmentSharingPolicyWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [EquipmentSharingPolicyWhereUniqueInput],
  })
  set?: Array<EquipmentSharingPolicyWhereUniqueInput>;
}

export { EquipmentSharingPolicyUpdateManyWithoutOrganizationsInput as EquipmentSharingPolicyUpdateManyWithoutOrganizationsInput };
