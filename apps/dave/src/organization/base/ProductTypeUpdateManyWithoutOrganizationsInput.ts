/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ProductTypeWhereUniqueInput } from "../../productType/base/ProductTypeWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class ProductTypeUpdateManyWithoutOrganizationsInput {
  @Field(() => [ProductTypeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ProductTypeWhereUniqueInput],
  })
  connect?: Array<ProductTypeWhereUniqueInput>;

  @Field(() => [ProductTypeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ProductTypeWhereUniqueInput],
  })
  disconnect?: Array<ProductTypeWhereUniqueInput>;

  @Field(() => [ProductTypeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ProductTypeWhereUniqueInput],
  })
  set?: Array<ProductTypeWhereUniqueInput>;
}

export { ProductTypeUpdateManyWithoutOrganizationsInput as ProductTypeUpdateManyWithoutOrganizationsInput };
