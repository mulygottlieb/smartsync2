/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { OrganizationAwardWhereUniqueInput } from "./OrganizationAwardWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { OrganizationAwardUpdateInput } from "./OrganizationAwardUpdateInput";

@ArgsType()
class UpdateOrganizationAwardArgs {
  @ApiProperty({
    required: true,
    type: () => OrganizationAwardWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => OrganizationAwardWhereUniqueInput)
  @Field(() => OrganizationAwardWhereUniqueInput, { nullable: false })
  where!: OrganizationAwardWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => OrganizationAwardUpdateInput,
  })
  @ValidateNested()
  @Type(() => OrganizationAwardUpdateInput)
  @Field(() => OrganizationAwardUpdateInput, { nullable: false })
  data!: OrganizationAwardUpdateInput;
}

export { UpdateOrganizationAwardArgs as UpdateOrganizationAwardArgs };
