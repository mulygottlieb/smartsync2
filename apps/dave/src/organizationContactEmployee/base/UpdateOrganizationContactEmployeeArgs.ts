/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { OrganizationContactEmployeeWhereUniqueInput } from "./OrganizationContactEmployeeWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { OrganizationContactEmployeeUpdateInput } from "./OrganizationContactEmployeeUpdateInput";

@ArgsType()
class UpdateOrganizationContactEmployeeArgs {
  @ApiProperty({
    required: true,
    type: () => OrganizationContactEmployeeWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => OrganizationContactEmployeeWhereUniqueInput)
  @Field(() => OrganizationContactEmployeeWhereUniqueInput, { nullable: false })
  where!: OrganizationContactEmployeeWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => OrganizationContactEmployeeUpdateInput,
  })
  @ValidateNested()
  @Type(() => OrganizationContactEmployeeUpdateInput)
  @Field(() => OrganizationContactEmployeeUpdateInput, { nullable: false })
  data!: OrganizationContactEmployeeUpdateInput;
}

export { UpdateOrganizationContactEmployeeArgs as UpdateOrganizationContactEmployeeArgs };
