...
const CREATE_INPUT = {
  ...
  tasksProofOfCompletionType: "exampleTasksProofOfCompletionType",
  isTasksLinkedEnabled: "true",
  isTasksNotifyLeftEnabled: "true",
  tasksNotifyLeftPeriodDays: 42,
  isTasksAutoCloseEnabled: "true",
  tasksAutoClosePeriodDays: 42,
  isTasksAutoArchiveEnabled: "true",
  tasksAutoArchivePeriodDays: 42,
...
describe("OrganizationTaskSetting", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: OrganizationTaskSettingService,
          useValue: service,
        },
      ],
  ...
  test("POST /organizationTaskSettings", async () => {
    await request(app.getHttpServer())
      .post("/organizationTaskSettings")
      .send(CREATE_INPUT)
  ...
  test("GET /organizationTaskSettings", async () => {
    await request(app.getHttpServer())
      .get("/organizationTaskSettings")
  ...
  test("GET /organizationTaskSettings/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/organizationTaskSettings"}/${nonExistingId}`)
  ...
  test("GET /organizationTaskSettings/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/organizationTaskSettings"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /organizationTaskSettings existing resource", async () => {
