/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { RolePermissionWhereInput } from "./RolePermissionWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { RolePermissionOrderByInput } from "./RolePermissionOrderByInput";

@ArgsType()
class RolePermissionFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => RolePermissionWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => RolePermissionWhereInput, { nullable: true })
  @Type(() => RolePermissionWhereInput)
  where?: RolePermissionWhereInput;

  @ApiProperty({
    required: false,
    type: [RolePermissionOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [RolePermissionOrderByInput], { nullable: true })
  @Type(() => RolePermissionOrderByInput)
  orderBy?: Array<RolePermissionOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { RolePermissionFindManyArgs as RolePermissionFindManyArgs };
