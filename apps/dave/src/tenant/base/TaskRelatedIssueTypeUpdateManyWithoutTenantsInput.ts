/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { TaskRelatedIssueTypeWhereUniqueInput } from "../../taskRelatedIssueType/base/TaskRelatedIssueTypeWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class TaskRelatedIssueTypeUpdateManyWithoutTenantsInput {
  @Field(() => [TaskRelatedIssueTypeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [TaskRelatedIssueTypeWhereUniqueInput],
  })
  connect?: Array<TaskRelatedIssueTypeWhereUniqueInput>;

  @Field(() => [TaskRelatedIssueTypeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [TaskRelatedIssueTypeWhereUniqueInput],
  })
  disconnect?: Array<TaskRelatedIssueTypeWhereUniqueInput>;

  @Field(() => [TaskRelatedIssueTypeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [TaskRelatedIssueTypeWhereUniqueInput],
  })
  set?: Array<TaskRelatedIssueTypeWhereUniqueInput>;
}

export { TaskRelatedIssueTypeUpdateManyWithoutTenantsInput as TaskRelatedIssueTypeUpdateManyWithoutTenantsInput };
