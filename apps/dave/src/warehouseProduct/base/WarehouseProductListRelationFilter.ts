/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { WarehouseProductWhereInput } from "./WarehouseProductWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class WarehouseProductListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => WarehouseProductWhereInput,
  })
  @ValidateNested()
  @Type(() => WarehouseProductWhereInput)
  @IsOptional()
  @Field(() => WarehouseProductWhereInput, {
    nullable: true,
  })
  every?: WarehouseProductWhereInput;

  @ApiProperty({
    required: false,
    type: () => WarehouseProductWhereInput,
  })
  @ValidateNested()
  @Type(() => WarehouseProductWhereInput)
  @IsOptional()
  @Field(() => WarehouseProductWhereInput, {
    nullable: true,
  })
  some?: WarehouseProductWhereInput;

  @ApiProperty({
    required: false,
    type: () => WarehouseProductWhereInput,
  })
  @ValidateNested()
  @Type(() => WarehouseProductWhereInput)
  @IsOptional()
  @Field(() => WarehouseProductWhereInput, {
    nullable: true,
  })
  none?: WarehouseProductWhereInput;
}
export { WarehouseProductListRelationFilter as WarehouseProductListRelationFilter };
